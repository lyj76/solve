...
def poison_data(all_edges):
    edges_list = []
    for frame in all_edges:
        for edge in all_edges[frame]:
            edges_list.append(edge)
    n_edges = len(edges_list)
    print(f">> 正在毒化数据: 总边数 {n_edges}")
    poisoned_indices = random.sample(range(n_edges), int(n_edges * 0.05))
    for idx in poisoned_indices:
        orig_meas = np.array(edges_list[idx]["measurement"])
        m = np.array(edges_list[idx]["measurement"])
        m[:2] *= 50.0 
        edges_list[idx]["measurement"] = m.tolist()
        edges_list[idx]["info_mat"] *= 100.0 
        edges_list[idx]["is_poisoned"] = True
    print(f">> 毒化完成: 注入 {len(poisoned_indices)} 个致命离群点。")
    return edges_list
...
def run_pgo_dual(filename, max_itr=5):
    ...
    edges_flat = poison_data(all_edges)
        ...
        for edge in edges_flat:
            ...
            if DEBUG_MODE and edge.get("is_poisoned", False) and itr==0 and not use_robust_kernel:
                if random.random() < 0.05:
                    print(f"   [DEBUG] 毒化边 {i}->{j} | 误差 Norm: {error_norm:.2f} (预期应很大)")
            weight = 1.0
            if use_robust_kernel:
                weight = compute_huber_weight(error_norm, threshold=5.0)
                if weight < 0.1: outlier_count += 1
    ...
    for itr in range(max_itr):
        ...
        print(f"迭代 {itr}: Numpy Chi2={err_np:.2e} | Smart Chi2={err_ss:.2e} (抑制 {n_out} 个异常点)")
    ...
    print("\n" + "="*80)
    print("                对抗实验总结: 鲁棒性与稳定性报告")
    print("="*80)
    print(f"{'指标':<20} | {'Numpy (受干扰)':<20} | {'SmartSolver (鲁棒)'}")
    print("-" * 80)
    print(f"{'平均位姿漂移 (RMSD)':<20} | {rmsd_np:<20.4f} m | {rmsd_ss:.4f} m")
    print(f"{'精度提升倍数':<20} | {'Baseline':<20} | {rmsd_np/(rmsd_ss+1e-6):.1f}x")
    print("-" * 80)
    
    plt.figure(figsize=(15, 10))
    plt.plot(x_gt[:, 0], x_gt[:, 1], color='black', linestyle='--', label='Ground Truth', alpha=0.5)
    plt.plot(x_np.reshape(-1,3)[:, 0], x_np.reshape(-1,3)[:, 1], color='red', label=f'Numpy (RMSD:{rmsd_np:.1f}m)', alpha=0.6)
    plt.plot(x_ss.reshape(-1,3)[:, 0], x_ss.reshape(-1,3)[:, 1], color='#32CD32', label=f'SmartSolver (RMSD:{rmsd_ss:.1f}m)', linewidth=2)
    plt.title("Visual Proof: SmartSolver vs Numpy on Poisoned Intel Dataset")
    plt.legend()
    plt.axis('equal')
    os.makedirs('output_results', exist_ok=True)
    plt.savefig("output_results/experiment_poisoned_intel.png", dpi=200)
    print(f"\n[成功] 对抗实验大图已生成: output_results/experiment_poisoned_intel.png")
